...
import type { FlowControlVariant, WizardAction } from '@/lib/types'

interface VariantOption {
  id: FlowControlVariant
  title: string
  diameters: string
  description: string
  features: string[]
...
const OPTIONS: VariantOption[] = [
  {
    ...
    description:
      'Pre-set orifice plate housed in an HDPE chamber. Simple, passive flow restriction sized to a design head.',
    features: [
      'Fixed orifice plate',
      'Discharge derived from head + orifice',
      'Drains down from ground level',
    ],
  },
  {
    id: 'ROTEX',
    ...
    description:
      'Passive vortex flow regulator. Higher discharge for the same head than an orifice; ideal for adoptable highway drainage.',
    features: [
      'Vortex regulator (no moving parts)',
      'Factory-fitted in HDPE chamber, or supplied standalone for concrete',
      'Curved or flat surface compatibility',
    ],
  },
]

export function FCVariantSelect() {
  const { state, dispatch } = useWizardContext()
  const data = state.productData?.kind === 'flow-control' ? state.productData.data : null
