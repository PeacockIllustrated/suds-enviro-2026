...
export default function ReviewLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Store author in sessionStorage for the review tool to read
      sessionStorage.setItem('se-reviewer-name', author)
      router.push('/review/tool')
    } catch (err) {
        ...
        <form onSubmit={handleSubmit}>
          <div className="rounded-xl border border-border bg-white p-6 shadow-sm">
            ...
            <div className="relative mb-5">
              <User className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-muted/50" />
              <select
                value={author}
                onChange={(e) => setAuthor(e.target.value)}
                className="w-full appearance-none rounded-lg border border-border bg-white py-3 pl-10 pr-4 text-[14px] text-ink outline-none transition-colors focus:border-blue"
              >
